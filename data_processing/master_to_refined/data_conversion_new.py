...
# IN_WEB_VIRTUAL resolved at runtime — handles both "websiteURL" (old master)
    # and "website URL" (new master) and any future whitespace variants.
    _viw_target = "please provide the website url where more information about your virtual program / service can be found"
    IN_WEB_VIRTUAL = next(
        (c for c in master.columns
         if re.sub(r"\s+", " ", c.replace("\xa0", " ").replace("\t", " ")).strip().lower()
            .replace("websiteurl", "website url") == _viw_target),
        None,
    )
    if IN_WEB_VIRTUAL is None:
        print("[WARN] Could not find virtual-service URL column — virtual webpage URLs will be blank.")
    else:
        print(f"[INFO] Virtual URL column detected: {IN_WEB_VIRTUAL!r}")
    IN_WEB_PHYSICAL = "Program/Service Webpage:"
    IN_WEB_FALLBACK = "Webpage:"

    def get_series(col: str) -> pd.Series:
            # Exact match first; then fall back to norm-match to handle
            # whitespace/typo variants between template versions.
            actual_col = col if col in master.columns else next(
                (c for c in master.columns if norm_header(c) == norm_header(col)), None
            )
            if actual_col is None:
                return pd.Series(pd.NA, index=master.index, dtype="object")
            s = master[actual_col].astype("object")
